...
exports.newInsert = (req) => {
    try {
        console.log("in reqqqqqqq", req)
        mode = (req.Tx_modeOfEntry.slice(0, 2)).toString() === "90" ? "swipe" :
                (req.Tx_modeOfEntry.slice(0, 2)).toString() === "01" ? 'keyEntry' :
                ((req.Tx_modeOfEntry.slice(0, 2)).toString() === "91" | "07") ? 'chip' : '';
        // mode = (req.Tx_modeOfEntry.slice(0, 2)).toString() === "90" ? "swipe" : (req.Tx_modeOfEntry.slice(0, 2)).toString() === "07" ? 'keyEntry' : '';
        paymentstatus = (req.Tx_status === "00") ? "approve" : "decline";
        // pinmode = change.fullDocument.is_pin_txn ? 'keyentry' : '';
        // chip = change.fullDocument.is_emv_txn ? 'chip' : ''
        schema.details.findOne({
            "EPI": req.EPI,
            "date": {
                $gte: dateFns.startOfDay(req.date),
                $lte: dateFns.endOfDay(req.date)
            }
        }).then(data => {
            ...
            if (data !== null) {
                schema.details.updateOne({
                    ...
                    $inc: {
                        "Tx_count": 1,
                        [`type_of_card.${req.type_of_card}.cardCount`]: 1,
                        [`Tx_status.${paymentstatus}.cardCount`]: 1,
                        [`Tx_type.${req.Tx_type}.cardCount`]: 1,
                        [`Tx_modeOfEntry.${mode}.cardCount`]: 1

                    },
                    $set: {
                        "Tx_volume": parseInt(data.Tx_volume) + parseInt(req.Tx_volume),
                        [`type_of_card.${req.type_of_card}.cardVolume`]: parseInt(data.type_of_card[req.type_of_card].cardVolume) + parseInt(req.Tx_volume),
                        [`Tx_status.${paymentstatus}.cardVolume`]: parseInt(data.Tx_status[paymentstatus].cardVolume) + parseInt(req.Tx_volume),
                        [`Tx_type.${req.Tx_type}.cardVolume`]: parseInt(data.Tx_type[req.Tx_type].cardVolume) + parseInt(req.Tx_volume),
                        [`Tx_modeOfEntry.${mode}.cardVolume`]: parseInt(data.Tx_modeOfEntry[mode].cardVolume) + parseInt(req.Tx_volume)
                    }
                ...
                const newValue = new schema.details({
                    ...
                    [`type_of_card.${req.type_of_card}.cardVolume`]: parseInt(req.Tx_volume),
                    [`Tx_status.${paymentstatus}.cardVolume`]: parseInt(req.Tx_volume),
                    [`Tx_type.${req.Tx_type}.cardVolume`]: parseInt(req.Tx_volume),
                    [`Tx_modeOfEntry.${mode}.cardVolume`]: parseInt(req.Tx_volume),
                    "Tx_count": 1,
                    "Tx_volume": req.Tx_volume,
                    "date": req.date
                })
